---
user-guide-title: Getting Started with AEM and Adobe Target 
product: AEM
version: 6.5
sub-product: 
audience: 
doc-type: 
feature: 
topics:
audience: 
activity: 
index: y
---
 
# Getting Started with AEM Headless{#getting-started-with-aem-headless}

+ [Overview](./overview.md)
+ [Implementation]{#aem-target-implementation}
  + [Introduction](./implementation.md)
  + [Using Launch and Adobe I/O](./using-launch-adobe-io.md)
  + [Using AEM Legacy Cloud Services](./using-aem-cloud-services.md)
+ [Use Case 1: Geo Based Personalization](./personalization-use-case-1.md)
+ [Use Case 2: Home Page Redesign and Redirects](./personalization-use-case-2.md)
+ [Use Case 3: Personalization using Experience Composer](./personalization-use-case-3.md)
+ [Authoring Content Services Pages](./personalization-faqs.md)
