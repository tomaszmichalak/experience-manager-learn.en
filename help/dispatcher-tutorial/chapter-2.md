...
We have had some experiments with NFS – but NFS introduces huge performance issues due to content locking. So this does actually decreases the overall performance.
...
If you are also concerned about the latency a visitor experiences, consider using a content delivery network, cache re-fetching, preemptive cache warming, setting a grace time as described in Part 1 of this series or refer to some advanced ideas in Part 3.
...
This however comes with a price.  First, taking out one leg for maintenance is quite cumbersome. Actually, this is what this scheme was designed for; to be more resilient and stay up and running by all means possible. I have seen complicated maintenance plans on how deal with this. Reconfigure the Dispatcher 2 first, removing the cross connection. Restarting Dispatcher 2. Shutting down Dispatcher 1, update Publish 1, … and so on. You should consider carefully if that scales up to more than two legs and if you come to the same conclusion, that it actually adds complexity, costs and a formidable source for human error, It would be best to automate this. So better check, if you actually have the resources to include this automation task in your project schedule. While you might save some hardware costs, you might spend double in IT staff.
...
We do not advise that you use this scheme as a default option. However, if you decide to use it, you will want to carefully assess risks and hidden costs and plan to include configuration automation as part of your project.
